/*
  # Corrigir taxa de serviço para 2%

  1. Alterações
    - Atualizar função calculate_total_cost para usar 2% em vez de 5%
    - Manter compatibilidade com dados existentes
    - Corrigir cálculo de taxa de serviço
...
-- Atualizar a função calculate_total_cost para usar 2% de taxa de serviço
...
BEGIN
  ...
  -- Add 2% service fee to get total cost (ALTERADO DE 5% PARA 2%)
  NEW.total_cost := subtotal * 1.02;

  RETURN NEW;
EXCEPTION
  WHEN OTHERS THEN
    -- In case of any error, set total_cost to sum of known costs without OPME, plus 2% service fee
    subtotal := (
      COALESCE(NEW.icu_daily_cost, 0) * icu_days +
      COALESCE(NEW.ward_daily_cost, 0) * ward_days +
      COALESCE(NEW.room_daily_cost, 0) * room_days +
