/*
  # Sistema de Usuários e Acompanhamento de Orçamentos

  1. New Tables
    - `user_profiles` - Perfis de usuários (médicos e secretárias)
      - `id` (uuid, primary key)
      - `user_id` (uuid, foreign key to auth.users)
      - `name` (text)
      - `role` (text, 'doctor' or 'secretary')
      - `crm` (text, nullable - apenas para médicos)
      - `specialty` (text, nullable - apenas para médicos)
      - `doctor_id` (uuid, nullable - apenas para secretárias)
      - `created_at` (timestamp)

    - `user_surgery_requests` - Pedidos de cirurgia dos usuários
      - `id` (uuid, primary key)
    ...
    - `user_budget_tracking` - Acompanhamento de orçamentos
      - `id` (uuid, primary key)
      - `surgery_request_id` (uuid, foreign key)
      - `budget_id` (uuid, nullable, foreign key to budgets)
      - `status` (text, default 'in_progress')
      - `user_approval` (text, nullable)
