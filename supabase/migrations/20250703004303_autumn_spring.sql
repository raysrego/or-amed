/*
  ...
  1. New Tables
    - `user_profiles`
      ...
      - `name` (text)
      - `role` (text, doctor or secretary)
      - `crm` (text, for doctors)
      - `specialty` (text, for doctors)
      - `doctor_id` (uuid, references user_profiles for secretaries)
      - `created_at` (timestamp)
    
    - `user_surgery_requests`
      - `id` (uuid, primary key)
    ...
    - `user_budget_tracking`
      - `id` (uuid, primary key)
      - `surgery_request_id` (uuid, references user_surgery_requests)
  ...
  2. Security
    ...
    - Add policies for doctor-secretary relationships

  3. Functions and Triggers
    - Auto-create budget tracking when surgery request is created
    - Update budget tracking when budget status changes
*/
...
DROP TRIGGER IF EXISTS trigger_create_budget_tracking ON user_surgery_requests;
DROP TRIGGER IF EXISTS trigger_update_budget_tracking_status ON budgets;
DROP FUNCTION IF EXISTS create_budget_tracking();
DROP FUNCTION IF EXISTS update_budget_tracking_status();
